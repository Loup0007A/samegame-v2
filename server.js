...
const ADMIN_PASSWORD_HASH = sha256(ADMIN_SALT + 'MotDePasseAdmin123#@');

// Ensure admin account exists
(function seedAdmin() {
  const exists = db.get('accounts').find({ username: ADMIN_USERNAME }).value();
  if (!exists) {
...
function verifyPassword(password, salt, storedHash) {
  return sha256(salt + password) === storedHash;
}
...
app.post('/api/auth/register', (req, res) => {
  ...
  db.get('accounts').push({
    id: accountId, username, passwordHash: hash, salt,
...
app.post('/api/auth/login', (req, res) => {
  ...
  if (username === ADMIN_USERNAME) {
    if (sha256(ADMIN_SALT + password) !== ADMIN_PASSWORD_HASH) return res.status(401).json({ error: 'Invalid credentials' });
  } else {
    if (!verifyPassword(password, account.salt, account.passwordHash)) return res.status(401).json({ error: 'Invalid credentials' });
  }
...
function requireAdmin(req, res, next) {
  ...
  if (token !== sha256(ADMIN_SALT + 'session-valid')) return res.status(403).json({ error: 'Forbidden' });
  next();
}
...
app.post('/api/admin/auth', (req, res) => {
  const { username, password } = req.body;
  if (username !== ADMIN_USERNAME || sha256(ADMIN_SALT + password) !== ADMIN_PASSWORD_HASH) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  const token = sha256(ADMIN_SALT + 'session-valid');
